...
{
    const
    asar_module = require('asar')
    ;                                                               // fs.readFileSync = readFileSyncEnhanced
    return (
        //  ensuring asar func is only called once per inititializtion
        asar = function ()
        {
            return asar_module
        }
...
function
readFileSyncEnhanced (
    file,
    _opts
){
    const
    opts = _get_read_opts_by_type[typeof _opts, _opts]
    ;
...
writeFileMem (
    ...
    const
    string = (data.byteLength !== undefined)
        ?   data.toString('utf8')
        :   data
    ;
